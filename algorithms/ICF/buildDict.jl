    ...
    for user in keys(models)
        for item in keys(models[user])
            try
                ...
                res[item] = Dict{Int64, Int64}()
            end
            res[item][user] = models[user][item]
        end
    end
...
modelsT = transpose(models)

save(model_file, "nor_models", models, "popularities", pop, "pop_thresholds", popTh, "glb_avg", glb_avg, "u_biases", biases, "tr_nor_models", modelsT)
